...
var concat = function (tmpdir, imgs, cb){
	...
	i.write(path,function (err,val){
		...
		console.log("Prewriting cropped images for concatenation: "+path);
    ...
    var f = imgs[0];
    var r = _.rest(imgs);
    var paths = r.map(function (i, idx){ return pWrite(i, tmpdir+"/crop-"+idx+".png")});
    Q.all(paths).then(function (writtenImages){
	for(var i = 0; i < writtenImages.length; i++) {
	    f.append(writtenImages[i]);
	}
	var finishedConcat= tmpdir+"/finished_concat"+generateUUID()+".png";
	f.write(finishedConcat, function (){
	    console.log("Wrote finishedConcat to "+finishedConcat);
